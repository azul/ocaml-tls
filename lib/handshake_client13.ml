...
let answer_server_hello state ch (sh : server_hello) secrets raw log =
  ...
  | Ciphersuite.DHE_PSK ->
    ( match keyshare, pre_shared_key, state.config.Config.cached_session with
      | Some (group, keyshare), Some psk, Some e ->
        ...
        let ss = e.resumption_secret
        and es = Nocrypto.Dh.shared group secret keyshare in
  ...
  | Ciphersuite.DHE_RSA ->
    ...
    | Some (group, keyshare) ->
      ...
      let ss = Nocrypto.Dh.shared group secret keyshare in
      let es = ss in
...
let answer_certificate_verify (state : handshake_state) (session : session_data) exts es ss cv raw log =
  verify_digitally_signed state.protocol_version ~context_string:"TLS 1.3, server CertificateVerify" state.config.Config.hashes cv log session.peer_certificate >>= fun () ->
